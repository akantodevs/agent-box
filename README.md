...
2. **Create `docker-compose.yml`** in your project directory (empty dirs work too —
   ...
   services:
     agent:
       ...
       ports:
         - 8081:8081 # web terminal — one browser tab per session
         - 8086:8082 # session administration page
       environment:
         TTYD_USER: "admin"
         TTYD_PASSWORD: "admin"
         TTYD_PUBLIC_PORT: "8081" # must match the terminal mapping above
         CLAUDE_MODEL: "opus"
       volumes:
...
3. **Run it:**
   ...
   Open the **session administration page** at **http://localhost:8086** (login
   `admin` / `admin`) and click **+ New session** — it opens a terminal tab on
   http://localhost:8081. Complete the one-time Claude login there, and start
...
- **Claude Code in a box** — runs as the non-root `claude` user inside a Debian
  ...
  web terminal on container port **8081**.
...
- **Many sessions, one box** — a [session administration page](#sessions) on container
  port **8082** lists every conversation in the volume and opens each one in its own
...
Browse to **http://localhost:8086** — the session administration page — and
...
terminal on **http://localhost:8085**.

> This repository's own `docker-compose.yml` publishes the terminal on host port
> **8085** and the admin page on **8086**; the quickstart above publishes the terminal on
> **8081**. Whatever you map, `TTYD_PUBLIC_PORT` must name the terminal's *host* port —
> it is what the admin page builds its links from.
...
not one per container. The two servers divide the work: the admin page decides *which*
...
- **Opening the terminal port directly starts a *new* session.** The terminal URL with
...
- **Closing the tab ends the session.** The tab *is* the session's terminal, so when it
...
services:
  agent:
    ...
    environment:
      TTYD_USER: "admin"
      TTYD_PASSWORD: "admin"
      TTYD_PUBLIC_PORT: "8081" # host port of the terminal mapping above
...
- `container_name` and the host ports are fixed, so only one agent-box runs at a time;
  change them all (and keep `TTYD_PUBLIC_PORT` matching the terminal's new host port) if
  you need two projects up simultaneously.
...
| `TTYD_PUBLIC_PORT`            | `8085`            | The **host** port the terminal is published on (the left half of the `8085:8081` mapping). The container can't discover this itself, and the admin page needs it to build its links — so it must match the `ports:` entry, or every link on the page points at the wrong port. |
| `ADMIN_PUBLIC_PORT`           | `8086`            | The host port the admin page is published on. Only used for the "Container ready" line in the container log; the page itself works regardless. |
...
| Piece                                     | Role                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docker-compose.yml`                      | Defines the `agent` service: build, the `agent-box:latest` image, the two published ports (terminal `8081`, admin page `8082`), env vars, and the volume mounts. The `name:` field pins the Compose project name.                                                                                                                                                                          |
| `agent-box/Dockerfile`                    | Builds the image: Debian + Node.js + Claude Code CLI + docker CLI + Terraform + kubectl + ttyd + Playwright MCP with headless Chromium, plus everyday CLI tools (`ps`/`pkill`, `jq`, `less`, `nc`, `dig`, `unzip`, `wget`, `tree`, Python with pip/venv, ...), and creates the non-root `claude` user. Ships a healthcheck that probes **both** servers (8081 and `/healthz` on 8082). The Claude Code auto-updater is disabled — the image owns the version. |
| `agent-box/ep.sh`                         | Entrypoint (runs as **root**): fixes ownership, seeds first-run config, grants `claude` access to the Docker socket, installs plugins, then launches ttyd and — in a restart loop, so a crash there never costs you the terminal — the session administration server.                                                                                                                      |
...
| `agent-box/scripts/start_claude.sh`       | Runs the Claude Code process for one tab: `claude --model "$CLAUDE_MODEL" --resume <id>`, or `--session-id <fresh uuid>` for a new session. Appends `--remote-control <name>-<suffix>` when `REMOTE_CONTROL_NAME` is set.                                                                                                                                                                  |
| `agent-box/scripts/sessions.py`           | The session administration server on container port `8082` (stdlib only, runs as `claude`): the page itself at `/`, `GET /api/sessions`, `POST /api/sessions/<uuid>/delete`, and an unauthenticated `/healthz` for the healthcheck. Everything else is behind the same basic-auth credentials as ttyd.                                                                                     |
...
| `agent-box/scripts/sessions_page.html`    | The admin page's UI. `sessions.py` bakes `TTYD_PUBLIC_PORT` into it at startup so the rows can link to the terminal.                                                                                                                                                                                                                                                                      |
...
4. It launches **ttyd** on port `8081` with `-a` (so a tab can name its session as
   `?arg=<session-id>`) and no client limit. Every connection runs
   `launch_session.sh`, which validates that id and then starts one Claude Code
   process as `claude`. There is no global "one client" cap any more — the rule is per
   session: one tab per conversation, enforced by the launcher.
5. It launches the **session administration server** on port `8082` as `claude`,
...
- **"Refusing to resume: session ... is already open in another tab":** that session is
  ...
  admin page.
- **The admin page's links point at the wrong port:** `TTYD_PUBLIC_PORT` doesn't match
  the terminal's host port in `ports:`. Fix it and recreate the service — the port is
  baked into the page when the server starts.
